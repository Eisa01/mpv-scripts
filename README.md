...
- [**SmartCopyPaste Script 2.0**](https://github.com/Eisa01/mpv-scripts#smartcopypaste-script-20)
- [**SmartCopyPaste-II Script 2.0**](https://github.com/Eisa01/mpv-scripts#smartcopypaste-ii-script-20)
- [**SmartHistory Script 1.5**](https://github.com/Eisa01/mpv-scripts#smarthistory-script-15)
...
Basically,  the **II**  version is enhanced with a bookmark feature, copying a video will bookmark the video and time, while pasting will access the bookmark (even if clipboard is cleared). For installation, download *`SmartCopyPaste-II-2.0.lua`* file into your `mpv/scripts/` directory. 
...
- MAC OS (requires pbcopy and pbpaste).
- Linux OS (requires xclip).
# SmartHistory Script 1.5
SmartHistory is a script for mpv media player, the script adds a smart history functionality to mpv. It logs videos that you opened into a log file along with the time you have reached on each video; log file location for Windows OS: `%APPDATA%\mpv\mpvHistory.log`, for Linux and MAC OS: `~\.config\mpv\mpvHistory.log`. The script uses the log to provide you with various features. More details about SmartHistory are explained in the sections below. For installation, download *`SmartHistory-1.5.lua`* file into your `mpv/scripts/` directory. 
...
SimpleUndo is a script for mpv media player, the script adds a simple undo functionality into mpv. If you accidentally seek/jump to a different time in the video, press undo <kbd>ctrl</kbd>+<kbd>z</kbd> to return to your previous time and vice-versa. For installation, download *`SimpleUndo-2.5.lua`* file into your `mpv/scripts/` directory. 
